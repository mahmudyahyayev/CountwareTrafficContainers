...
namespace CountwareTraffic.Services.Devices.Infrastructure
{
    public class ProcessOutboxScheduleHandler : ICommandHandler<ProcessOutboxSchedule>
    {
        ...
        public ProcessOutboxScheduleHandler(DeviceDbContext context, IQueueService queueService, ILogger<ProcessOutboxScheduleHandler> logger)
        {
            _context = context;
        ...
        public async Task HandleAsync(ProcessOutboxSchedule command)
        {
            ...
            const string sql = @"SELECT [OutboxMessage].[Id],
                                        [OutboxMessage].[Type],
	                                    [OutboxMessage].[Data]
                                   FROM [device.app].[OutboxMessages] AS [OutboxMessage] 
	                                    WHERE [IsTryFromQueue] = 1 AND [LastException] IS NOT NULL";

            var messages = await connection.QueryAsync<OutboxMessageDto>(sql);
            var messagesList = messages.AsList();

            const string sqlUpdateProcessedDate = @"UPDATE [device.app].[OutboxMessages] SET [ProcessedDate] = @Date,  [LastException] = NULL WHERE [Id] = @Id";
