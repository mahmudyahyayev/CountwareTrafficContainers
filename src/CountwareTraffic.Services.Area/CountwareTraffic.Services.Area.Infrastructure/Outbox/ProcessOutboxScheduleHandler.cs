...
namespace CountwareTraffic.Services.Areas.Infrastructure
{
    public class ProcessOutboxScheduleHandler : ICommandHandler<ProcessOutboxSchedule>
    {
        ...
        public ProcessOutboxScheduleHandler(AreaDbContext context, IQueueService queueService, ILogger<ProcessOutboxScheduleHandler> logger)
        {
            _context = context;
        ...
        public async Task HandleAsync(ProcessOutboxSchedule command)
        {
            ...
            const string sqlUpdateProcessedDate = @"UPDATE [area.app].[OutboxMessages] SET [ProcessedDate] = @Date,  [LastException] = NULL WHERE [Id] = @Id";
    ...
    {
        public static readonly Assembly Application = typeof(SubAreaCreated).Assembly;
    }
}
