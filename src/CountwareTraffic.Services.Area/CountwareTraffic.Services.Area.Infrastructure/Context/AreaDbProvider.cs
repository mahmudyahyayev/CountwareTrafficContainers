...
{
    public class AreaDbProvider : InterceptorDefination, IScopedSelfDependency
    {
        ...
        public AreaDbProvider(IQueueEventMapper queueEventMapper, IIdentityService identityService, IQueueService queueService, ICommandDispatcher commandDispatcher)
        {
            ...
            OnDomainEventRaiseAfterSuccess += () =>
            {
                ...
                {
                    var queueEvents = _queueEventMapper.MapAll(raisable.Events, GetUserId());

                    foreach (var queueEvent in queueEvents)
                    {
                        string message = null;

                        try { _queueService.Publish(queueEvent); }
        ...
        public override Guid GetUserId() => _identityService.UserId;
    }
}
