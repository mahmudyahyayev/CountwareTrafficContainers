    ...
    public class SendTemplatedSmsHandler : ICommandHandler<SendTemplatedSms>
    {
        ...
        public async Task HandleAsync(SendTemplatedSms command)
        {
            ...
            try
            {
                ...
                var jsonPhoneNumbers = Sensormatic.Tool.Common.TextJsonExtensions.Serialize(command.Template.PhoneNumbers);
                var jsonResult = Sensormatic.Tool.Common.TextJsonExtensions.Serialize(result);

                var smsLog = CountwareTraffic.WorkerServices.Sms.Data.SmsLog.Create(jsonUserIds, jsonPhoneNumbers, message, jsonResult, templatedSms.IsOtp, SmsType.Templated.Id);
