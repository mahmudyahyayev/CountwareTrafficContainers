...
namespace CountwareTraffic.Services.Companies.Infrastructure
{
    public class SaveChangesInterceptor : ISaveChangesInterceptor, IDisposable
    {
        ...
        public InterceptionResult<int> SavingChanges(DbContextEventData eventData, InterceptionResult<int> result)
        {
            ...
            {
                if (entry.Entity.GetType().GetInterfaces().Any(a => a == typeof(Sensormatic.Tool.Efcore.IDeletable)))
        ...
        void InterceptBefore(DbContext context)
        {
            foreach (var entry in entityEntries)
            {
                ...
                foreach (Type intface in entry.Entity.GetType().GetInterfaces().Where(w => w.GetInterfaces().Any(a => a == typeof(Sensormatic.Tool.Efcore.IInterceptor))))
                {
